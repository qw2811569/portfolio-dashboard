...
#### ~~G-J~~ ✅ 全部完成（02fd601, 3951ccc）

### 新一輪任務（Claude 2026-04-02 第三輪指派）

**開始前先 `git pull origin main`。**

#### K. 補動態事件行事曆測試

task E 的 FinMind TaiwanStockNews 動態事件源沒有測試覆蓋。補上：

1. `fetchFinMindNewsEvents()` — mock FinMind API 回傳，驗證法說/股東會/除權息關鍵字篩選正確
2. 驗證不相關新聞被過濾（例如：一般財報新聞不含「法說」不應變成事件）
3. Gemini 事件篩選修正 — 驗證 Gemini 事件現在會按用戶持股篩選（你在 task E 修的 bug）

**位置：** 新建 `tests/api/event-calendar.test.js`

#### L. FinMind 資產負債表 + 現金流量表加入 dossier context

Codex 已在 adapter 加入 `fetchBalanceSheet(code)` 和 `fetchCashFlowStatement(code)`，但 dossier context 組裝還沒接上。

在 `buildDailyHoldingDossierContext()` 中，當 dossier 有 `finmind.balanceSheet` 或 `finmind.cashFlow` 時，追加到分析 context：

```javascript
// 資產負債表摘要
if (finmind.balanceSheet?.length > 0) {
  const latest = finmind.balanceSheet[0]
  // 找到 totalAssets, totalLiabilities, shareholderEquity
  parts.push(`資產負債: 總資產${latest.totalAssets}M, 負債比${latest.debtRatio}%`)
}

// 現金流摘要
if (finmind.cashFlow?.length > 0) {
  const latest = finmind.cashFlow[0]
  parts.push(`現金流: 營業${latest.operatingCF}M, 投資${latest.investingCF}M`)
}
```

**注意：** FinMind 的 BalanceSheet/CashFlow 用 `type`/`value` schema（跟 FinancialStatements 一樣），需要 pivot。確認 Codex 的 adapter 如何 transform 這些數據再決定欄位名。

**位置：** `src/lib/dossierUtils.js`

#### M. 供應鏈視覺化元件

`supplyChain.json` 現在有 20 檔持股的完整上下游關係。建一個簡單的供應鏈展示元件：

- 在持股詳細頁（或 research panel）展示上游→本公司→下游的關係
- 如果上游/下游是我們的持股（有 code），顯示為可點擊的連結
- 顯示 competitors（如果有）

**範例 UI：**

```
上游: 合晶科技(6182) → [力積電(6770)] → 下游: 聯詠(3034), Micron, Tata
競爭: 台積電, 聯華電子, 世界先進
```
