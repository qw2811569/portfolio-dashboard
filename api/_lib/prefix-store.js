...
function normalizeRelativePath(
  value,
  { label, allowEmpty = false, preserveTrailingSlash = false } = {}
) {
  const normalized = String(value || '')
    .trim()
    .replace(/^\/+/, '')
...
function pickSampleKeys(keys = [], sampleSize, randomImpl = Math.random) {
  ...
  while (remaining.length > 0 && samples.length < sampleSize) {
    const index = Math.max(
      0,
      Math.min(remaining.length - 1, Math.floor(randomImpl() * remaining.length))
    )
...
async function scanPrefixPage(storeName, descriptor, options = {}) {
  ...
  return {
    ...
    nextCursor:
      items.length > pageLimit && pageItems.length > 0 ? pageItems[pageItems.length - 1].key : null,
  }
}
...
async function deleteManyFromStore(storeName, descriptors, options = {}) {
  ...
  if (storeName === 'gcs') {
    ...
    if (!bucketName) {
      throw new Error(
        `GCS bucket is required for ${descriptors[0]?.keyspace || 'prefix-store'} deletes`
      )
    }
    ...
    return {
      deletedKeys: result.deletedKeys.map((key) =>
        logicalKeyFromRelativePath(
          stripKnownPrefix(key, [descriptors[0].gcsPrefix]),
...
export function createPrefixStore({
  ...
  function buildListDescriptor(params = {}) {
    ...
    if (cursorKey && !cursorKey.startsWith(prefixKey)) {
      throw new Error(
        `[${loggerPrefix}] cursor ${cursorKey} does not belong to prefix ${prefixKey}`
      )
    }
    ...
    async read(key, options = {}) {
      ...
      scheduleBackgroundTask(async () => {
        try {
          ...
          logger.warn?.(
            `[${loggerPrefix}] shadow read compare failed for ${descriptor.key}:`,
            error
          )
        }
      })
    ...
    async list(params = {}, options = {}) {
      ...
      scheduleBackgroundTask(async () => {
        try {
          ...
          logger.warn?.(
            `[${loggerPrefix}] shadow list compare failed for ${descriptor.key}:`,
            error
          )
        }
      })
