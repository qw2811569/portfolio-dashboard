...
export function parsePrimaryStore(
  ...
  throw new Error(`[${loggerPrefix}] ${envName} must be "vercel" or "gcs"; received "${candidate}"`)
}
...
function normalizeListedKeys(items = []) {
  return Array.from(
    new Set(
      (Array.isArray(items) ? items : []).map((item) => resolveListedItemKey(item)).filter(Boolean)
    )
...
function buildDescriptorResolver({
  ...
  const listDescriptorFactory =
    typeof resolveListDescriptor === 'function' ? resolveListDescriptor : null
