...
- `npm run healthcheck` 也會讀 `.tmp/vite-dev.log`（2026-04-28 前舊名 `vercel-dev.log`），補看 Vite frontend 訊號與 HMR invalidation 警告
...
3. 檢查 `.tmp/vite-dev.log`（舊名 `.tmp/vercel-dev.log`，2026-04-28 disconnect 後改名）看 Vite frontend 訊號
...
| ~~`vercel.json` crons~~（已 retire） | 2026-04-28 後改 systemd timer · 排程在 `deploy/systemd/cron-*.timer` |
