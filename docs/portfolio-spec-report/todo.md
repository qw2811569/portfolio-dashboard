...
| O02 | Provider usage threshold report                                    | Daily   | daily check; warn/escalate thresholds documented   | usage snapshot                                                                    | provider quota exhausted without warning        | pairs with B10/Q10            |
...
| O07 | GCP VM (bigstock + jcv-dev) monthly cost + deploy frequency review | Monthly | month-end or abnormal spike                        | VM bill summary + push note · Vercel build minutes 2026-04-28 起 disconnected = 0 | cost creep breaks Rule 4 intent                 | trend review, not policing    |
...
| O02 | 1. Daily provider-usage report has a stable command.<br>2. Warn/escalate thresholds are documented.<br>3. Evidence can be archived.             | `node scripts/audit-kb-availability.mjs && rg -q "warn.*escalate" docs .tmp`                        | `—`                                           | `codex-then-review`                     | `L8`                     |
...
| O07 | 1. Monthly VM cost / deploy-frequency review has a ledger.<br>2. Review points back to Rule 4 push discipline.<br>3. Cost review doc exists.    | `test -f docs/ops/vm-cost-ledger.md && rg -q "VM bill\\                                             | deploy frequency" docs/ops/vm-cost-ledger.md` | `M-U1 dashboard access`                 | `user-manual-then-codex` | `L8` |
