...
export const DEFAULT_STATE_PATH = path.resolve('.tmp/migration-state/tracked-stocks.json')
export const DEFAULT_LOCK_PATH = path.resolve('.tmp/migration-state/tracked-stocks.lock')
const TRACKED_STOCKS_PREFIX = 'tracked-stocks/'
const TRACKED_STOCKS_KEY_PATTERN = /^tracked-stocks\/([^/]+)\/latest\.json$/
...
function createItemFromKey(key, bucketName) {
  const match = TRACKED_STOCKS_KEY_PATTERN.exec(String(key || '').trim())
...
export async function loadKeyInventory({
  ...
  if (!token) {
    throw new Error('BLOB_READ_WRITE_TOKEN is required for tracked-stocks migration reads')
  }
  ...
  do {
    ...
    for (const blob of Array.isArray(page?.blobs) ? page.blobs : []) {
      ...
      const item = createItemFromKey(pathname, bucketName)
...
function mergeInventory(existingItems = [], inventory = []) {
  const merged = new Map((Array.isArray(existingItems) ? existingItems : []).map((item) => [item.key, item]))
...
export async function runMigration(rawOptions = {}, deps = {}) {
  ...
  try {
    ...
    if (inventory.length === 0) {
      throw new Error('No tracked-stocks keys were found in the Blob prefix')
    }
...
if (isDirectRun) {
  main().catch((error) => {
    console.error('[migrate-tracked-stocks-to-gcs] failed:', error?.message || error)
    process.exitCode = 1
  })
}
