...
export default function Header(props) {
  const {
    cloudSync, saved, refreshPrices, refreshing, copyWeeklyReport,
    exportLocalBackup, backupFileInputRef, importLocalBackup,
    priceSyncStatusTone, priceSyncStatusLabel, activePriceSyncAt, lastUpdate,
    pc, displayedTotalPnl, displayedRetPct,
    activePortfolioId, switchPortfolio, ready, portfolioSwitching, portfolioSummaries,
    createPortfolio, viewMode, exitOverview, openOverview,
    showPortfolioManager, setShowPortfolioManager, renamePortfolio, deletePortfolio,
    OWNER_PORTFOLIO_ID, overviewTotalValue, portfolioNotes, setPortfolioNotes,
    PORTFOLIO_VIEW_MODE, OVERVIEW_VIEW_MODE,
    urgentCount, todayAlertSummary, TABS, tab, setTab,
  } = props;
  ...
  const card = {
    ...
    padding: "12px 14px", boxShadow: `${C.insetLine}, ${C.shadow}`,
  };
  ...
  return h("div", {
    className: "app-shell",
    style: {
      background: `${C.shell}f0`, borderBottom: `1px solid ${C.borderSoft}`,
      padding: "10px 14px 0", position: "sticky", top: 0, zIndex: 10,
      boxShadow: C.shellShadow, backdropFilter: "blur(16px) saturate(160%)",
    ...
    h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 } },
      h("div", { style: { display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 0 } },
        ...
        h("button", { className: "ui-btn", onClick: refreshPrices, disabled: refreshing, style: {
          ...
          ...ghostBtn, cursor: refreshing ? "not-allowed" : "pointer",
        }}, refreshing ? "同步中..." : "⟳ 收盤價"),
        h("button", { className: "ui-btn", onClick: copyWeeklyReport, style: {
          background: C.lavBg, color: C.lavender, border: `1px solid ${alpha(C.lavender, A.strongLine)}`,
          ...ghostBtn,
        }}, "📋 週報"),
        h("button", { className: "ui-btn", onClick: exportLocalBackup, style: {
          background: C.oliveBg, color: C.olive, border: `1px solid ${alpha(C.olive, A.strongLine)}`,
          ...ghostBtn,
        }}, "備份"),
        h("button", { className: "ui-btn", onClick: () => backupFileInputRef?.current?.click(), style: {
          background: C.subtle, color: C.textSec, border: `1px solid ${C.border}`,
          ...ghostBtn,
        }}, "匯入"),
        h("input", { ref: backupFileInputRef, type: "file", accept: "application/json,.json",
          onChange: importLocalBackup, style: { display: "none" } }),
        ...
        lastUpdate && !refreshing && h("span", { style: { fontSize: 9, color: C.textMute } },
    ...
    h("div", { style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 8 } },
      ...
      h("select", {
        ...
        style: {
          minWidth: 190, background: C.subtle, color: C.text, border: `1px solid ${C.border}`,
          borderRadius: 8, padding: "7px 10px", fontSize: 11, outline: "none",
      ...
      h("button", {
        className: "ui-btn", onClick: createPortfolio, disabled: !ready || portfolioSwitching,
        style: {
          background: C.cardBlue, color: C.blue, border: `1px solid ${alpha(C.blue, A.strongLine)}`,
          ...ghostBtn, cursor: !ready || portfolioSwitching ? "not-allowed" : "pointer",
        },
      ...
      h("button", {
        className: "ui-btn", onClick: () => setShowPortfolioManager(prev => !prev),
        style: {
          background: showPortfolioManager ? C.subtleElev : C.subtle,
          color: C.textSec, border: `1px solid ${showPortfolioManager ? C.borderStrong : C.border}`,
          ...ghostBtn,
        },
      }, showPortfolioManager ? "收合管理" : "管理組合"),
      activePortfolioId && h("span", { style: { fontSize: 10, color: C.textSec } },
    ...
    // Portfolio manager
    showPortfolioManager && h("div", { style: { ...card, marginBottom: 8, borderLeft: `3px solid ${alpha(C.teal, A.glow)}` } },
          ...
          h("div", { key: portfolio.id, style: {
            ...
            h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, flexWrap: "wrap" } },
              ...
              h("div", { style: { display: "flex", gap: 6, flexWrap: "wrap" } },
                (portfolio.id !== activePortfolioId || viewMode === OVERVIEW_VIEW_MODE) && h("button", {
                  className: "ui-btn", onClick: () => switchPortfolio(portfolio.id),
                  style: { background: C.cardBlue, color: C.blue, border: `1px solid ${alpha(C.blue, A.strongLine)}`, ...ghostBtn },
                }, "打開這組"),
                h("button", { className: "ui-btn", onClick: () => renamePortfolio(portfolio.id),
                  style: { background: C.cardAmber, color: C.amber, border: `1px solid ${alpha(C.amber, A.strongLine)}`, ...ghostBtn },
                }, "改名"),
                portfolio.id !== OWNER_PORTFOLIO_ID && h("button", { className: "ui-btn", onClick: () => deletePortfolio(portfolio.id),
                  style: { background: C.upBg, color: C.up, border: `1px solid ${alpha(C.up, A.strongLine)}`, ...ghostBtn },
                }, "刪除")
              )
            )
          )
        ...
        h("div", { style: { marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.borderSub}` } },
          ...
          h("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 } },
            h("div", null,
              ...
              h("input", {
                ...
                style: { width: "100%", background: C.subtle, border: `1px solid ${C.border}`, borderRadius: 7,
                  padding: "8px 10px", color: C.text, fontSize: 11, outline: "none", fontFamily: "inherit" },
              })
            ),
            h("div", null,
          ...
          h("div", null,
            ...
            h("textarea", {
              ...
              style: { width: "100%", background: C.subtle, border: `1px solid ${C.border}`, borderRadius: 7,
                padding: 8, color: C.text, fontSize: 11, resize: "vertical", minHeight: 72,
                outline: "none", fontFamily: "inherit", lineHeight: 1.7 },
            })
          )
        )
      :
        h("div", { style: { marginTop: 12, fontSize: 10, color: C.textMute, background: C.subtle,
          border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 10px", lineHeight: 1.7 } },
          "目前在全部總覽模式，資料維持唯讀。要編輯 notes，先用上方「打開」切回某個單一組合。"
        )
    ),

    // Urgent alerts
    viewMode !== OVERVIEW_VIEW_MODE && urgentCount > 0 && h("div", { style: {
      background: C.upBg, border: `1px solid ${alpha(C.up, A.line)}`,
      borderLeft: `3px solid ${C.up}`, borderRadius: 6,
      padding: "5px 10px", marginBottom: 8, fontSize: 10,
      color: C.up, lineHeight: 1.6, fontWeight: 500,
    }}, `今日 · ${todayAlertSummary}`),

    // Overview mode notice or tabs
    viewMode === OVERVIEW_VIEW_MODE ?
      h("div", { style: { background: C.subtle, border: `1px solid ${C.border}`, borderRadius: 8,
        padding: "8px 10px", marginBottom: 6, display: "flex", justifyContent: "space-between",
    ...
    :
      h("div", { className: "seg", style: { display: "flex", gap: 6, overflowX: "auto", padding: "2px 0 6px" } },
        TABS.map(t =>
          h("button", { className: "ui-btn", key: t.k,
