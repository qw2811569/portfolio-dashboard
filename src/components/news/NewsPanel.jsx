...
export function LogPanel({ tradeLog }) {
  if (!tradeLog || tradeLog.length === 0) {
    return h(Card, { style: { textAlign: "center", padding: "24px 14px" } },
      h("div", { style: { fontSize: 20, marginBottom: 6, opacity: 0.3 } }, "◌"),
      h("div", { style: { fontSize: 12, color: C.textMute, fontWeight: 400 } },
        "還沒有交易記錄", h("br"),
  ...
  return h("div", null,
    [...tradeLog].sort((a, b) => b.id - a.id).map(log =>
      h(Card, {
        ...
        h("div", { style: { fontSize: 11, color: C.textMute, marginBottom: 10 } },
 ...
 */
export function NewsEventCard({ event, onReview, isExpanded, onToggle }) {
...
export function NewsAnalysisPanel({
  ...
  const NE = newsEvents || [];
  const past = NE.filter(e => e.status === "closed" || e.status === "past").sort((a, b) => b.id - a.id);
  ...
  return h("div", null,
    // Review modal
    reviewingEvent && h(Card, {
      style: { marginBottom: 10, borderLeft: `3px solid ${alpha(C.up, "40")}` },
    },
      h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 } },
        h("div", { style: { ...lbl, color: C.up, marginBottom: 0 } }, `事件復盤：${reviewingEvent.title}`),
    ...
    // Pending events
    pending.length > 0 && h(Card, { style: { marginBottom: 10, borderLeft: `3px solid ${alpha(C.amber, "40")}` } },
      h("div", { style: { ...lbl, color: C.amber } }, `待處理 (${pending.length})`),
      pending.map(e => h(NewsEventCard, {
        key: e.id,
        event: e,
        onReview: (ev) => {
    ...
    // Tracking events
    tracking.length > 0 && h(Card, { style: { marginBottom: 10, borderLeft: `3px solid ${alpha(C.teal, "40")}` } },
      h("div", { style: { ...lbl, color: C.teal } }, `追蹤中 (${tracking.length})`),
      tracking.map(e => h(NewsEventCard, { key: e.id, event: e }))
    ),

    // Past events
    past.length > 0 && h(Card, { style: { marginBottom: 10 } },
      h("div", { style: { ...lbl } }, `歷史事件 (${past.length})`),
      past.map(e => h(NewsEventCard, {
        key: e.id,
        event: e,
