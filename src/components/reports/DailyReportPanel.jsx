...
export function DailyAnalysisEmpty({ onAnalyze, onStressTest, analyzing, stressTesting }) {
  return h(Card, {
    ...
    h("div", { style: { fontSize: 12, color: C.textSec, fontWeight: 500, marginBottom: 4 } }, "每日收盤分析"),
    h("div", { style: { fontSize: 10, color: C.textMute, marginBottom: 12, lineHeight: 1.6 } },
...
export function AnalyzingState({ step }) {
  return h(Card, {
    ...
    h("div", { style: { fontSize: 11, color: C.textMute, marginTop: 8 } },
      "取得股價 → 比對事件 → AI 策略分析 → 大腦進化"
    ),
    h("div", {
      style: { width: "100%", height: 3, background: C.borderSub, borderRadius: 2, marginTop: 12, overflow: "hidden" },
    },
      h("div", {
        style: {
          height: "100%",
...
export function StressResult({ result, onClose }) {
  ...
  return h(Card, {
    ...
    h("div", {
      ...
      h("div", { style: { fontSize: 13, fontWeight: 600, color: C.down } },
  ...
  return h(Card, {
    ...
    h("div", {
      ...
      h("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
        ...
        !expanded && report.anomalies?.length > 0 && h(Badge, { color: "amber" }, `異常 ${report.anomalies.length}`),
        !expanded && report.needsReview?.length > 0 && h(Badge, { color: "up" }, `復盤 ${report.needsReview.length}`)
      ),
  ...
  return h(Card, {
    ...
    correlations.map(ec =>
      h("div", {
        ...
        h("div", { style: { fontSize: 11, fontWeight: 500, color: C.text, marginBottom: 4 } }, ec.title),
 ...
 */
export function EventAssessments({ assessments, newsEvents, onNavigate }) {
    ...
    assessments.map((ea, i) => {
      ...
      return h("div", {
        ...
        h("div", { style: { display: "flex", gap: 8, alignItems: "center" } },
          h("span", { style: { fontSize: 9, color: C.textMute } }, `信心度 ${Math.round((ea.confidence || 0) * 100)}%`),
          ea.suggestClose && ea.suggestCloseReason && h("span", {
            style: { fontSize: 9, color: C.amber },
          }, ea.suggestCloseReason)
        ),
        ea.suggestClose && h(Button, {
          onClick: (ev) => {
            ev.stopPropagation();
...
export function BrainAuditSection({ brainAudit }) {
  ...
  return h(Card, {
    ...
    sections.map(section => {
      ...
      return h("div", { key: section.key, style: { marginBottom: 8 } },
        h("div", {
        ...
        rows.map((item, idx) =>
          h("div", {
            key: `${section.key}-${idx}`,
            style: { marginBottom: 6, background: C.subtle, borderRadius: 7, padding: "9px 11px" },
          },
            h("div", {
              style: { fontSize: 10, fontWeight: 600, color: C.textSec, marginBottom: 3 },
            }, item.text || item.id || "未命名規則"),
            item.reason && h("div", { style: { fontSize: 10, color: C.textSec, marginBottom: 2 } }, item.reason),
            h("div", { style: { display: "flex", flexWrap: "wrap", gap: 8 } },
              item.confidence != null && h("span", { style: { fontSize: 9, color: C.textMute } }, `信心度 ${item.confidence}%`),
              item.lastValidatedAt && h("span", { style: { fontSize: 9, color: C.textMute } }, `最近驗證 ${item.lastValidatedAt}`),
              item.staleness && h("span", { style: { fontSize: 9, color: C.textMute } }, `狀態 ${item.staleness}`),
              item.nextStatus && h("span", { style: { fontSize: 9, color: C.textMute } }, `建議轉為 ${item.nextStatus}`)
            ),
            (item.evidenceRefs || []).length > 0 && h("div", {
...
export function NeedsReviewSection({ needsReview, onNavigate, onExpand }) {
  ...
  return h(Card, {
    ...
    needsReview.map(e =>
      h("div", { key: e.id, style: { marginBottom: 8 } },
        h("div", { style: { fontSize: 11, fontWeight: 500, color: C.text } }, e.title),
        h("div", { style: { fontSize: 10, color: C.textMute } },
...
export function DailyReportPanel({
  ...
  return h("div", null,
    // Empty state
    !dailyReport && !analyzing && h(DailyAnalysisEmpty, {
