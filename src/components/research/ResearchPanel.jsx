 ...
 */
export function ResearchHeader({ onEvolve, onRefresh, researching, reportRefreshing, reportRefreshStatus }) {
  return h(Card, {
    ...
    h("div", { style: { fontSize: 11, color: C.textSec, lineHeight: 1.7, marginBottom: 10 } },
    ...
    h("div", { style: { display: "flex", gap: 6, marginBottom: 10 } },
      h(Button, {
        ...
        style: {
          ...
          background: researching ? C.subtle : `linear-gradient(135deg,${C.fillTomato},${C.fillChoco})`,
    ...
    reportRefreshStatus && h("div", { style: { fontSize: 10, color: C.textMute, marginBottom: 8 } }, reportRefreshStatus)
  );
}
 ...
 */
export function StockResearchButtons({ holdings, onResearch, researching, researchTarget, STOCK_META, IND_COLOR }) {
  return h("div", null,
    h("div", { style: { fontSize: 9, color: C.textMute, marginBottom: 4 } }, "個股研究選擇（3 輪迭代）："),
  ...
  return h(Card, {
    ...
    h("div", {
      ...
      h("div", null,
        ...
        h("div", { style: { fontSize: 11, color: C.textSec, lineHeight: 1.7 } },
    ...
    h("div", { style: { display: "grid", gap: 7 } },
      dataRefreshRows.slice(0, 5).map(item =>
        h("div", {
          ...
          h("div", null,
            h("div", { style: { fontSize: 11, color: C.text, fontWeight: 500 } }, `${item.name} (${item.code})`),
            h("div", { style: { fontSize: 9, color: C.textMute, marginTop: 3 } },
              `目標價：${item.targetStatus} · 財報：${item.fundamentalStatus}`
            )
          ),
          h(Button, {
            onClick: () => onResearch("single", holdings.find(h => h.code === item.code)),
...
export function ResearchProgress({ researching, researchTarget, holdings }) {
  ...
  return h(Card, {
    ...
    h("div", { style: { marginTop: 10, height: 3, background: C.subtle, borderRadius: 2, overflow: "hidden" } },
      h("div", {
        style: {
          height: "100%",
          background: C.teal,
...
export function ResearchHistory({ history, onSelect, selectedId }) {
  ...
  return h(Card, null,
    h("div", { style: { ...lbl, display: "flex", justifyContent: "space-between", alignItems: "center" } },
      h("span", null, "歷史研究記錄"),
      h("span", { style: { fontSize: 9, color: C.textMute, fontWeight: 400 } }, `${history.length} 筆`)
    ),
    history.map((r, i) =>
      h("div", {
        key: r.timestamp || i,
        ...
        h("div", null,
          h("span", { style: { fontSize: 12, color: r.mode === "evolve" ? C.up : C.text } },
...
export function ResearchPanel({
  ...
  return h("div", null,
    ...
    !researchResults && !researching && (!researchHistory || researchHistory.length === 0) && h(Card, {
      style: { textAlign: "center", padding: "24px" },
    },
      h("div", { style: { fontSize: 11, color: C.textMute, lineHeight: 1.8 } },
        "點擊上方按鈕開始第一次深度研究。", h("br"),
