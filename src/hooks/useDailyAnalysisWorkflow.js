...
import { buildKnowledgeDataAvailability } from '../lib/knowledgeAvailability.js'
import {
...
export function useDailyAnalysisWorkflow({
  ...
  const runDailyAnalysis = useCallback(
    async ({ triggerSource = 'manual' } = {}) => {
      if (analyzing) return
      setAnalyzing(true)
      ...
      try {
        ...
        const analysisVersion = existingTodayReport
        ...
        try {
          ...
          const dailyDossiers = buildAnalysisDossiers({
            changes,
            ...
            analysisDossiers.map((dossier) => [
              ...
              findHistoricalAnalogs(
                { code: dossier.code, name: dossier.name, sector: dossier?.meta?.industry },
                {
                  ...
                  title:
                    (dossier?.events?.pending || [])[0]?.title || dossier?.thesis?.summary || '',
