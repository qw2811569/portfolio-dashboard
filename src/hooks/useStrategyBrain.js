...
import { OWNER_PORTFOLIO_ID, PORTFOLIO_VIEW_MODE, BRAIN_VALIDATION_CASE_LIMIT } from "./constants.js";
import {
...
/**
 * Create empty brain validation store
 */
const createEmptyBrainValidationStore = () => ({
  version: 1,
  cases: [],
});
...
export const useStrategyBrain = ({
  ...
  const addValidationCase = useCallback((newCase) => {
    ...
    setBrainValidation(prev => {
      ...
      const exists = cases.some(c => 
        c?.id === newCase.id || 
        (c?.text === newCase.text && c?.verdict === newCase.verdict)
      );
   ...
   */
  const mergeBrainAudit = useCallback((nextBrain, currentBrain) => {
   ...
   */
  const ensureAuditCoverage = useCallback((currentBrain, dossiers = null) => {
   ...
   */
  const attachEvidenceRefs = useCallback((evidenceRefs, defaultLastValidatedAt = null) => {
   ...
   * Save brain to cloud
   */
  const saveBrainToCloud = useCallback(async (brainData) => {
    ...
    try {
      ...
      setSaved("⚠️ 策略大腦雲端同步失敗");
      setTimeout(() => setSaved(""), 3000);
    }
  }, [canUseCloud, setSaved]);
