...
import { createAppendLogStore, inferAppendLogKeyspaceId } from '../../api/_lib/append-log-store.js'
import {
  appendBlobJsonLine,
...
describe('api/_lib/append-log-store.js', () => {
  it('retries a CAS conflict and preserves the concurrently appended line', async () => {
    ...
    const result = await store.appendLine('logs/daily-snapshot-2026-04.jsonl', '{"line":"c"}', {
  ...
  it('uses monthly rotation keys for Class 3 wrappers', () => {
    ...
    expect(inferAppendLogKeyspaceId('logs/daily-snapshot-2026-04.jsonl')).toBe('daily_snapshot_log')
  })
