...
import { createFlatDataPathResolver, createHybridStore } from '../../api/_lib/hybrid-store.js'
...
describe('api/_lib/hybrid-store.js', () => {
  ...
  it('supports the four cutover shapes for local-mandatory writes and remote routing', async () => {
    ...
    for (const testCase of cases) {
      ...
      const storedPayload = JSON.parse(
        await readFile(resolveDataPath('research/2330/123.json'), 'utf8')
      )
  ...
  it('can disable local promotion for remote fallback reads', async () => {
    ...
    await expect(stat(resolveDataPath('strategy-brain.json'))).rejects.toMatchObject({
      code: 'ENOENT',
    })
    expect(payload).toEqual({ source: 'vercel' })
  })
