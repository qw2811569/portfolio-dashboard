...
describe('api/_lib/prefix-store.js', () => {
  ...
  it('supports the four cutover shapes for exact reads/writes and prefix listing', async () => {
    ...
    for (const testCase of cases) {
      ...
      await store.write(
        '2026-04-25/research-index.json',
        { ok: true },
        {
          token: 'blob-token',
          putImpl,
      ...
      expect(gcsListPrefixImpl, testCase.name).toHaveBeenCalledTimes(testCase.expected.gcsListCalls)
    }
  })
  ...
  it('documents the logical cursor race when a concurrent insert lands before the cursor', async () => {
    ...
    const listImpl = vi
      .fn()
      .mockResolvedValueOnce({
        blobs: [{ pathname: 'snapshot/research/a.json' }, { pathname: 'snapshot/research/c.json' }],
        cursor: 'vercel-page-2',
      })
      .mockResolvedValueOnce({
      ...
      .mockResolvedValueOnce({
        blobs: [{ pathname: 'snapshot/research/b.json' }, { pathname: 'snapshot/research/d.json' }],
        cursor: null,
      })
  ...
  it('records deleteMany shadow divergence when the secondary key is already gone', async () => {
    ...
    const result = await store.deleteMany(
      ['2026-04-24/research-index.json', '2026-04-24/portfolio-me-research-history.json'],
      {
        token: 'blob-token',
        storagePolicyOverride: {
          primary: 'gcs',
        ...
        headImpl: vi.fn().mockResolvedValueOnce({ etag: 'etag-1' }).mockResolvedValueOnce(null),
  ...
  it('samples shared list keys and records read divergence when content differs', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      expect.stringContaining(
        'shadow sampled read divergence for snapshot/research/2026-04-24/research-index.json'
      )
    )
  })
})
