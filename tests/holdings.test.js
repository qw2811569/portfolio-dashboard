...
function normalizeHoldingMetrics(item, overridePrice = null) {
  ...
  const value = Number.isFinite(price) && Number.isFinite(qty)
    ? Math.round(price * qty * 100) / 100
    : 0;
    
  const costValue = Number.isFinite(cost) && Number.isFinite(qty)
    ? Math.round(cost * qty * 100) / 100
    : 0;
    
  const pnl = Number.isFinite(value) && Number.isFinite(costValue)
    ? Math.round((value - costValue) * 100) / 100
    : 0;
    
  const pct = Number.isFinite(costValue) && costValue !== 0 && Number.isFinite(pnl)
